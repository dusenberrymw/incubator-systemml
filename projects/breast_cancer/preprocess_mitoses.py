...
def create_mask(im, coords, size):
    ...
    for stage, cases in [('train', train), ('val', val)]:
      for case in cases:
        ...
        for region_im in region_ims:
          ...
          os.makedirs(save_path, exist_ok=True)  # create if necessary
          for row, col in coords:
            # centered mitosis patch
          ...
          os.makedirs(save_path, exist_ok=True)  # create if necessary
          mask = create_mask(im, coords, patch_size)
          for i in range(num_normal * len(coords)):
...
if __name__ == "__main__":
  ...
  parser.add_argument("--num_normal", type=int, help="number of normal patches to extract for \
      each mitosis (default: `num_rand_translations+1`)")
...
def test_create_mask():
  ...
  coords = [(50, 40)]
  size = 32
  mask = create_mask(im, coords, size)
